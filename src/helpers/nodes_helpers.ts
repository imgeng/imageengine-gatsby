...
export function node_create(node_object: any, base_url: string, tokenized_url: string, ie_delivery_address: MaybeString,  directives: IEDirectives, mime_type?: string | undefined) {
    let { actions, node } = node_object;
    let parent_type = node_object?.node?.internal.type;
    let is_file = parent_type === "File";
    let clean_distribution = ie_delivery_address ? ie_delivery_address.replace(/\/+$/, "") : ""
	...
	ie_delivery_address: clean_distribution,
